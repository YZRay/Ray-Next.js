...
author: Ray
excerpt: 紀錄一下學習使用 view transition api 並實做出一些動畫效果
tags:
  - javascript
---
...
![alt can i use 圖片](https://imgur.com/wCpIIch.jpeg)
...
::view-transition ├─ ::view-transition-group(root) │ └─
::view-transition-image-pair(root) │ ├─ ::view-transition-old(root) │ └─
::view-transition-new(root) └─ ::view-transition-group(new-element) └─
::view-transition-image-pair(new-element) ├─ ::view-transition-old(new-element)
└─ ::view-transition-new(new-element)
```

## 實際使用 startViewTransition()

<CodeEmbed
  id="BaEdqWo?default-tab=css%2Cresult&theme-id=dark"
...
<CodeEmbed
  id="OJGjWdE?default-tab=css%2Cresult&theme-id=dark"
...
const { top, left } = toggleRef.current.getBoundingClientRect(); // 用 ref 來取得當前按鈕的位置
const right = window.innerWidth - left;
const bottom = window.innerHeight - top;
const maxRadius = Math.hypot(Math.max(left, right), Math.max(top, bottom)); // 計算出按鈕到頁面最遠的半徑距離，也就是直角三角形最長的邊
...
![alt 計算最遠距離圖片](https://imgur.com/Kk2h9xi.jpeg)
...
[使用 View Transitions API 构建优雅的动画效果 💢💢💢 - 掘金 (juejin.cn)](https://juejin.cn/post/7282889267028770877)
